...
class SemiSupBenchmark:
    def __init__(self, supervised_tr_stream, unsupervised_tr_stream, test_stream, valid_stream, image_size, num_classes, num_exps, exp_classes_list):
        ...
        self.exp_classes_list = exp_classes_list
...
class UnsupervisedDataset(Dataset):
    def __init__(self, data, transforms, num_views, crop_transforms=None, num_crop=0):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        if self.num_crop > 0:
            for _ in range(self.num_crop):
                crops.append(self.crop_transforms(input_tensor))

        return views, crops, label
...
def get_benchmark_label_delay(dataset_name : str,
                        ...
                        seed : int = 42,
                        unsup_anticipate_ratio : float = 0.5,
                        drop_last_exp : bool = False,
                            ) -> SemiSupBenchmark:
    """
    ...
    - eval_transform: Transformation function to apply on the evaluation data
    ...
    return SemiSupBenchmark(supervised_tr_stream, unsupervised_tr_stream, test_stream, valid_stream, image_size, num_classes, num_exps, exp_classes_list)   
