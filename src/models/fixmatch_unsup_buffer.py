...
class FixMatchUnsupBuffer(nn.Module):
    def __init__(self,
             num_classes: int,
             encoder: nn.Module,
        ...
        super(FixMatchUnsupBuffer, self).__init__()
    ...
    def forward(self, s_x1, s_x2, s_y, u_curr_x_weak, u_curr_x_strong, u_buff_x_weak, u_buff_strong):
        ...
        # Concat unsupervised current and buffer data
        if u_buff_x_weak is not None and u_buff_strong is not None:
            u_x_weak = torch.cat((u_curr_x_weak, u_buff_x_weak))
            u_x_strong = torch.cat((u_curr_x_strong, u_buff_strong))
        else:
            u_x_weak = u_curr_x_weak
            u_x_strong = u_curr_x_strong

        s_batch_size = s_x1.size(0)
        u_batch_size = u_x_weak.size(0)

        unsup_sup_ratio = u_batch_size // s_batch_size
        

        inputs = interleave(
            torch.cat((s_x1, u_x_weak, u_x_strong)), 2*unsup_sup_ratio+1)
